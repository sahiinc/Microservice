...
public class StandardCrmApplication implements CrmApplication {
	...
	@Override
	public DetailedCustomerResponse findCustomerByIdentity(String identity) throws CustomerNotFoundException{
		...
		if (Objects.nonNull(customer.getPhoto()))
		   detailedCustomerResponse.setPhoto(new String(Base64.getEncoder().encode(customer.getPhoto())));
		else
	...
	@Override
	@Transactional
	public UpdateCustomerResponse updateCustomer(String identity, UpdateCustomerRequest request) throws CustomerNotFoundException {
		...
		if (Objects.nonNull(photo))
		   managedCustomer.setPhoto(Base64.getDecoder().decode(photo));
		...
		updateCustomerResponse.setPhoto(new String(Base64.getEncoder().encode(managedCustomer.getPhoto())));
		return updateCustomerResponse;
	}

	@Override
	@Transactional
	public PatchCustomerResponse patchCustomer(String identity, Map<String, Object> request) throws CustomerNotFoundException {
		var managedCustomer = customerJpaRepository.findById(identity)
				.orElseThrow(() -> new CustomerNotFoundException());
	...
	@Override
	@Transactional
	public DeleteCustomerResponse removeCustomerByIdentity(String identity) throws CustomerNotFoundException {
		var managedCustomer = customerJpaRepository.findById(identity)
				.orElseThrow(() -> new CustomerNotFoundException());
